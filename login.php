<?php
//подключение к БД
define("LINK", mysqli_connect("localhost","prokal","san0k2000"));
if (LINK == false){
...
if ($login != "" and $password != ""){
    if (($captcha -> success ?? false) == true){
        if (preg_match("/^[a-zA-Zа-яА-Я0-9@._-]+$/", $login)){
            ...
            if ($user != false) {
                $check = password_verify($password, $user -> password ?? false);
    
                if ($check == true) {
                    $hash = generateHash();
                    mysqli_query(LINK, "UPDATE `users` SET hash = '{$hash}' WHERE id = '{$user -> id}'");
                    setcookie("id", $user -> id, time() + 3600 * 24 * 30 * 12, "/juicedev/", "prokal.tyt.su", true, true);
...
<head>
    ...
    <title>Вход - JuiceDev</title>
    <script src="https://www.google.com/recaptcha/api.js?onload=onloadCallback&render=explicit" async defer></script>
    <script type="text/javascript">
        var onloadCallback = function() {
            grecaptcha.render('recaptcha', {'sitekey' : 'RECAPTCHA_PUBLIC'});
        };
    </script>
</head>
